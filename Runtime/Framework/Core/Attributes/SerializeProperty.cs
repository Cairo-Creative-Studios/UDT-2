...
public class SerializeProperty : PropertyAttribute
{
    ...
    public bool InBaseType { get; private set; }

    public SerializeProperty(string propertyName, bool inBaseType)
    {
        PropertyName = propertyName;
        InBaseType = inBaseType;
    }
}
...
public class SerializePropertyAttributeDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        {
            if(((SerializeProperty)attribute).InBaseType)
            {
                propertyFieldInfo = target.GetType().BaseType.GetProperty(((SerializeProperty)attribute).PropertyName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.DeclaredOnly);
            }
            else
            {
                propertyFieldInfo = target.GetType().GetProperty(((SerializeProperty)attribute).PropertyName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.DeclaredOnly);
            }
        }
